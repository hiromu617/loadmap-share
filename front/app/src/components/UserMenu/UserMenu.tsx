    ...
    <Menu as="div" className="relative inline-block text-left">
      ...
      >
        <Menu.Items className="absolute right-0 w-56 mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none p-3">
          <Menu.Item>
            {({ active }) => (
              <Link href={`/user/${currentUser.uid}`}>
                <a className={`${"text-gray-900"} text-left  w-full rounded-md  block px-2 py-2 text-md hover:bg-blue-400 hover:text-white`}>
                  Account settings
                </a>
              </Link>
            )}
          ...
          <Menu.Item>
            {({ active }) => (
              <button className={`${"text-gray-900"} text-left w-full rounded-md  block px-2 py-2 text-md hover:bg-blue-400 hover:text-white`}>
                Logout
              </button>
            )}
          </Menu.Item>
